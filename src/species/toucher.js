...
export default function toucherGremlin() {
	const { document: { body } } = global;

	const config = {
		touchTypes: ['tap', 'tap', 'tap', 'doubletap', 'gesture', 'gesture', 'gesture', 'multitouch', 'multitouch'],
		positionSelector() {
			return [
	 ...
	 * @param [rotation=0]
	 */
	const getTouches = function(center, points, rad, deg) {
		const [cx, cy] = center;

		// just one touch, at the center
		if (points === 1) return [{ x: cx, y: cy }];

		const radius = rad || 100;
		const degrees = deg !== null ? (deg * Math.PI / 180) : 0;
		const slice = 2 * Math.PI / points;

		return range(points)
			.map((point, idx) => {
				const angle = (slice * idx) + degrees;
				return {
	 ...
	 * @param type
	 */
	const triggerTouch = function(touches, element, type) {
		const event = document.createEvent('Event');

		event.initEvent(`touch${type}`, true, true);	

		const touchlist = touches.map((touch, idx) => {
	 ...
	 * @param done
	 */
	const triggerGesture = function(element, startPos, startTouches, gesture, done) {
		const interval = 10;
		...
		const gestureLoop = () => {
			...
			const rotation = isFinite(gesture.rotation) ? (gesture.rotation / loops * loop) : null;
	...
	const touchTypes = {
		...
		gesture(position, element, done) {
			const gesture = {
				distanceX: config.randomizer.integer({ min: 0, max: 200 }),
				distanceY: config.randomizer.integer({ min: 0, max: 200 }),
		...
		multitouch(position, element, done) {
			const points = config.randomizer.integer({ min: 2, max: config.maxTouches });
			const gesture = {
				...
				distanceX: config.randomizer.integer({ min: 0, max: 20 }),
				distanceY: config.randomizer.integer({ min: 0, max: 20 }),
	...
	const toucherGremlin = function(done) {
		...
		touchTypes[touchType](position, targetElement, function logGremlin(touches, details) {
			config.showAction && config.showAction(touches);
			config.logger && config.logger.log && config.logger.log('gremlin', 'toucher ', touchType, 'at', posX, posY, details);
			done();
		});
	};
