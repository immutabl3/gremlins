...
An updated version of [gremlins.js](https://github.com/marmelab/gremlins/) that works with webpack and incorporates a `pointerGremlin`

gremlins is a monkey testing library written in JavaScript, for Node.js and the browser. Use it to check the robustness of web applications by unleashing a horde of undisciplined gremlins.
...
`gremlins` provides several gremlin *species*: some click everywhere on the page, others enter data in form inputs, others scroll the window in every possible direction, etc.
...
Gremlins, just like mogwais, are simple JavaScript functions. If `gremlins` doesn't provide the gremlin that can break your application, it's very easy to develop it:
...
Everything in `gremlins` is configurable ; you will find it very easy to extend and adapt to you use cases.

## Installation

`> npm i @immutabl3/gremlins`
...
`gremlins` currently provides a few gremlins and mogwais:

* [clickerGremlin](src/species/clicker.js) clicks anywhere on the visible area of the document
* [pointerGremlin](src/species/pointer.js) emits pointer events anywhere on the visible area of the document
...
All the gremlins and mogwais provided by `gremlins` are *configurable functions*, i.e. you can alter the way they work by calling methods on them.
...
If you want the attack to be repeatable, you need to seed the random number generator. Gremlins depends on [Chance.js](http://chancejs.com/) for random data generation, so it supports seeding:
...
Note that if using default gremlins, there are [five type of gremlins](https://github.com/marmelab/gremlins/blob/master/src/main.js#L12). The previous example would give a 0 value to last gremlin specie.
...
Your feedback about the usage of gremlins in your specific context is valuable, don't hesitate to [open GitHub Issues](https://github.com/marmelab/gremlins/issues) for any problem or question you may have.
...
gremlins is licensed under the [MIT Licence](LICENSE), courtesy of [marmelab](http://marmelab.com).
